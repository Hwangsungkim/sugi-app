...
if check_login_and_user():
    ...
    with tabs[0]:
        ...
        qna_list = [
            ...
            "51. 서로의 매력을 한 단어로 표현한다면?", "52. 가장 좋아하는 스킨십 타이밍은 언제야?", "53. 만약 내가 강아지/고양이로 변한다면 어떻게 키워줄 거야?", "54. 나랑 같이 꼭 가보고 싶은 유명한 맛집이 있다면?", "55. 상대방의 옷 스타일 중 가장 마음에 단 코디는?",
    ...
    with tabs[5]:
        ...
        for i, t in enumerate(sorted_timeline):
            with st.expander(f"{t.get('date','')} : {t.get('event','')}"):
                try:
