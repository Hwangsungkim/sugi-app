...
def show_weather_effect(w_type):
    if w_type == "rain":
        effect_css = """
        .drop { position: fixed; background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(0, 100, 255, 0.8)); width: 2px; height: 12vh; top: -12vh; z-index: 9999; pointer-events: none; animation: fall 0.7s linear infinite; }
        ...
        effect_css = """
        .mist { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(150, 160, 180, 0.2); z-index: -99998; pointer-events: none; }
        .fog { position: fixed; background: rgba(255, 255, 255, 0.3); border-radius: 50%; filter: blur(50px); z-index: -99998; pointer-events: none; animation: drift 50s linear infinite; }
        ...
        effect_css = """
        .sun-flare { position: fixed; top: -15vh; right: -15vw; width: 60vw; height: 60vw; background: radial-gradient(circle, rgba(255,180,0,0.4) 0%, rgba(255,210,100,0.1) 60%, transparent 80%); border-radius: 50%; z-index: -99998; pointer-events: none; animation: pulse 6s ease-in-out infinite alternate; }
...
services = get_sheets()
if not services: st.error("🚨 구글 연동 실패!")
...
if check_login_and_user():
    ...
    with st.sidebar:
        st.markdown(f"""<div style="background:rgba(255,255,255,0.3); padding:15px; border-radius:15px; border:2px solid {accent_color}; text-align:center; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
        ...
        st.metric(label="함께한 시간", value=f"D + {days_passed}일")
    ...
    with st.expander("✏️ 공지사항 수정"):
        ...
        if st.button("공지 변경 확정 💾"):
            st.session_state.notice = new_notice; save_main_data(); st.rerun()
    ...
    with tabs[2]:
        ...
        tele_qs = [["평생 여름", "평생 겨울"], ["찍먹", "부먹"], ["강아지", "고양이"], ["연락 5시간", "여사친과 밥"]]
    ...
    with tabs[4]:
        ...
        with st.expander("✨ 새로운 추억 보관하기"):
            ...
            if "아이폰" in mode:
                ...
                ev_d = col1.date_input("날짜", key="ip_d"); ev_n = col2.text_input("제목", key="ip_n")
                if st.button("☁️ 업로드"):
                    if img_fs:
                ...
                if st.session_state.photo_cart:
                    ...
                    col1, col2 = st.columns([0.4, 0.6])
                    ev_d = col1.date_input("날짜", key="an_d"); ev_n = col2.text_input("제목", key="an_n")
    ...
    with tabs[5]:
        ...
        with st.form("t_form", clear_on_submit=True):
            ...
            if st.form_submit_button("기록"):
                st.session_state.timeline.insert(0, {"date": str(td), "event": te, "by": user_name_only})
    ...
    with tabs[7]:
        ...
        with st.form("cap_form"):
            ct = st.text_input("제목"); cd = st.date_input("개봉일"); cc = st.text_area("내용")
