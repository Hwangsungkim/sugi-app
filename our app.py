...
symbol = "✨"
divs = "".join([f'{symbol}' for _ in range(10 if w_type in ["rain", "snow"] else (3 if w_type == "cloud" else 1))])
st.markdown(f"{effect_css}{divs}", unsafe_allow_html=True)
...
st.markdown(f"""
    ...
    .user-girl {{ border-right:5px solid #FF85A2 !important; background:rgba(255,133,162,0.15) !important; text-align: right; }}
...
# 7. 📍 장소/기록 (🚨 [버그 픽스] 파이썬 기본 dict로 안정적 단어 추출)
with tabs[6]:
...
with tabs[8]:
    ...
    if st.button("돌리기! 🎲") and custom_opts: st.success(f"🎉 당첨: **{random.cho
